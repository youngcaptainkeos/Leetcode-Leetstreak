# 🚀 LeetStreak 24/7 Hosting Guide (Render + UptimeRobot)

This guide walks you through deploying the LeetStreak backend to Render for free and keeping it active **24 hours a day with 0 downtime and 0 cold starts**.

---

## 1. Push Code to GitHub

1. Repository on GitHub: `https://github.com/youngcaptainkeos/Leetcode-Leetstreak.git`.
2. Run the following commands in your project folder to push:

```bash
git add .
git commit -m "LeetStreak - Brand update & security enhancements"
...
3. Connect your `youngcaptainkeos/Leetcode-Leetstreak` GitHub repository.
4. Render will automatically detect [`render.yaml`](file:///c:/PDocuments/leetcode%20tracker/codestreak/render.yaml) and create two services:
   - 🐘 **`leetstreak-db`**: PostgreSQL Database
   - ⚡ **`leetstreak-api`**: Python FastAPI Web Service
5. Click **Apply**.
6. Once deployed, Render will provide your public URL (e.g., `https://leetstreak-api.onrender.com`).

Verify your live backend is running:
Open `https://leetstreak-api.onrender.com/api/health` in your browser.
...
2. Click **Add New Monitor**:
   - **Monitor Type**: `HTTP(s)`
   - **Friendly Name**: `LeetStreak 24/7 Ping`
   - **URL (or IP)**: `https://leetstreak-api.onrender.com/api/health`
...
```typescript
export const API_BASE = "https://leetstreak-api.onrender.com/api";
```
